...
function feeSchedule(ctx: DocContext): DocTemplate {
  return {
    ...
    body: [
      ...
      ``,
      `6. IF A PAYMENT CANNOT BE MADE`,
      `   • Tell staff before the due date. Asking for help early is not a rule violation and will not be held against the Resident.`,
      `   • The Residence may agree to a written payment plan. Any plan is put in writing and signed by both parties.`,
      `   • Largest unpaid balance allowed before residency is at risk: ____________`,
      `   • Written notice given before the Resident is asked to leave for non-payment: ____________ days`,
      `No Resident is asked to leave for money alone without staff first offering help to find other housing. The Residence cannot guarantee a placement, but will make referrals and record them.`,
      `What happens to belongings left behind is set out in the Residency & Occupancy Agreement.`,
      ``,
...
function leaseAgreement(ctx: DocContext): DocTemplate {
  return {
    ...
    body: [
      ...
      ``,
      `2. PROGRAM FEES`,
      `The Resident agrees to pay program fees of ${or(ctx.rent)}. Fees are due on the schedule communicated by the Residence. Every fee the Resident can be charged is listed on the signed Schedule of Fees & Refund Policy, and no fee may be charged that does not appear there.`,
      ``,
      ...
      `Where the reason is non-payment, the Residence will give ____________ days written notice and will offer referrals to other housing before the Resident's last day. Leaving the same day is reserved for conduct that puts someone in danger.`,
      ``,
      // Standard 5a requires the removal policy to be in the signed agreement, not just a liability disclaimer.
      `5. PERSONAL PROPERTY`,
      `The Residence is not responsible for lost, stolen, or damaged personal property while the Resident lives here. Residents are encouraged to safeguard their belongings.`,
      `If the Resident moves out or is asked to leave, anything left behind is handled this way:`,
      `   • The Resident has ____________ days from their last day to ask in writing for their belongings.`,
      `   • The Residence holds belongings for at least ____________ days from the last day.`,
      `   • After a written request, belongings are returned within ____________ days.`,
      `   • Medication left behind is disposed of under house policy and is never mailed.`,
      `Property still unclaimed after the holding period may be disposed of.`,
      ``,
